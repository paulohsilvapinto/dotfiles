...
add_theme_var() {
    if [ "$1" = "BRANCH" ] && {[ "$REPO_BRANCH" = "develop" ] || [ "$REPO_BRANCH" = "master" ]}; then
        eval STATUS="\$STATUS\$ZSH_THEME_GIT_PROMPT_BRANCH_CAUTION"    
    else
...
ZSH_THEME_GIT_PROMPT_BRANCH_CAUTION="%{$fg_bold[red]%}\ue0a0 \u2622 "
